...
export default function News({onUpgrade}: {onUpgrade: (idea: ProductUpgrade) => void}) {
  const [filter,setFilter] = useState("All");
  const {data,loading,error,refresh} = useResource<Feed>("/api/news",60000);
  ...
  return <div className="insight-page">
    ...
    {data && <>
      <div className="source-status">Checked {new Date(data.checked_at).toLocaleString()} · checks the server every minute</div>
      ...
      {(filter==="All" || filter==="Product lab") && <section className="product-lab"><div className="eyebrow">PRODUCT LAB · EDITORIAL SUGGESTIONS</div><h2>Build a better feedback loop</h2><p className="page-description">Ideas for improving this product, not news reports or trading recommendations.</p>{data.ideas.map((idea,i)=><article className="idea-card" key={idea.id}><div className="article-meta"><span>0{i+1} / PRODUCT IDEA</span><span>{idea.effort} effort</span></div><h3><button className="idea-title" onClick={()=>onUpgrade(idea)} aria-label={`Review upgrade: ${idea.title}`}>{idea.title} ↗</button></h3><p>{idea.detail}</p><div className="measure"><strong>How to evaluate it</strong><p>{idea.measure}</p></div><button className="upgrade-button" onClick={()=>onUpgrade(idea)}>Build this upgrade</button>{idea.related_url && <a className="related-link" href={idea.related_url} target="_blank" rel="noopener noreferrer">Related headline: {idea.related_title} ↗</a>}</article>)}</section>}
